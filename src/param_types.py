...
class RangeParamType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        try:
            ...
            if not (self._is_within_range(start) and self._is_within_range(end)):
                raise ValueError(f'Given range is not within allowed range: `{self.min_start}:{self.max_end}`')
    ...
    def _is_within_range(self, x: int):
        return (
            (not self.min_start or x >= self.min_start) and
            (not self.max_end or x <= self.max_end)
        )
